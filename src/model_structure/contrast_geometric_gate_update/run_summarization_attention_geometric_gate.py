...
# from modeling_bart_contrast import BartForConditionalGeneration
from modeling_bart_contrast_attention_geometric_gate import BartForConditionalGeneration
...
@dataclass
class ContrastSeq2SeqTrainingArguments(Seq2SeqTrainingArguments):
    tau: Optional[float] = field(
        default=0.07,
        metadata={
            "help": "hyper parameter to compute contrastive loss"
        },
    )
    do_eval: bool = field(
        default=False,
...
class ModelArguments:
    ...
    cache_dir: Optional[str] = field(
        default="autodl-fs/RHO-main/src/cache_dir",
    ...
    early_stopping_patience: Optional[int] = field(
        default=1,
        metadata={
            "help": "`metric_for_best_model` to stop training when the specified metric worsens for `early_stopping_patience` evaluation calls."}
    )
    ####################prefix####################
...
class DataTrainingArguments:
    ...
    neg_samples_num: Optional[int] = field(default=1)#负采样的负样本数量
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, ContrastSeq2SeqTrainingArguments))
    ...
    """
    改写下面这个函数，增加负采样的机制，直接在数据预处理的时候增加负采样的逻辑
    """
    def preprocess_function(examples):
        list_keys = [key for key in examples]
        ...
        ############################################################## 增加正负样本的判断符
        model_inputs_len = len(model_inputs['memory_bank'])
        model_inputs['neg_pos_label'] = [1] * (model_inputs_len)
        return model_inputs

    def shuffle_model_inputs(model_inputs):
        # 获取样本数量
        num_samples = len(next(iter(model_inputs.values())))  # 获取第一个键的长度
        ...
        for key in model_inputs.keys():
            model_inputs[key] = [model_inputs[key][i] for i in shuffled_indices]

        return model_inputs
    """
    负采样过程直接移动到数据预处理阶段
    """
    def preprocess_function_neg(examples):
        neg_samples_num = data_args.neg_samples_num
        list_keys = [key for key in examples]
        inputs = examples[text_column]
        targets = examples[summary_column]
        inputs = [prefix + inp for inp in inputs]
        ...
        for memory_bank in examples['memory_bank']:
            ...
            model_inputs['memory_bank'].append(memory_bank)
        ############################################################## 增加正负样本的判断符
        # print('model_inputs_len', model_inputs['memory_bank'])
        model_inputs_len = len(model_inputs['memory_bank'])
        model_inputs['neg_pos_label'] = [1] * (model_inputs_len)
        neg_input_entity_relation_ids = []
        neg_attention_masks = []
        neg_input_ids = []
        neg_labels = []
        neg_memory_bank = []
        neg_memory_bank_attention_mask = []

        neg_pos_label = [0] * (model_inputs_len * neg_samples_num)
        # print('examples len',model_inputs_len)
        for i in range(model_inputs_len):
            neg_input_entity_relation_ids.extend([model_inputs['input_entity_relation_ids'][i] for _ in range(neg_samples_num)])
            neg_input_ids.extend([model_inputs['input_ids'][i] for _ in range(neg_samples_num)])
            neg_labels.extend([model_inputs['labels'][i] for _ in range(neg_samples_num)])
            neg_attention_masks.extend([model_inputs['attention_mask'][i] for _ in range(neg_samples_num)])
            neg_sample_idx = random.sample([j for j in range(model_inputs_len) if j != i], neg_samples_num)  # 负采样的id
            for neg_idx in neg_sample_idx:
                neg_memory_bank_sample = model_inputs['memory_bank'][neg_idx]
                neg_memory_bank_attention_mask_sample = model_inputs['memory_bank_attention_mask'][neg_idx]
                neg_memory_bank.append(neg_memory_bank_sample)
                neg_memory_bank_attention_mask.append(neg_memory_bank_attention_mask_sample)
        # print('1',len(neg_memory_bank))
        # print('2',len(neg_memory_bank_attention_mask))
        # print('3',len(neg_pos_label))
        # print('4',len(neg_input_entity_relation_ids))
        # print('5',len(neg_input_ids))
        # print('6',len(neg_labels))
        model_inputs['neg_pos_label'].extend(neg_pos_label)
        model_inputs['input_entity_relation_ids'].extend(
            neg_input_entity_relation_ids)
        model_inputs['labels'].extend(neg_labels)
        model_inputs['input_ids'].extend(neg_input_ids)
        model_inputs['memory_bank'].extend(neg_memory_bank)
        model_inputs['memory_bank_attention_mask'].extend(
            neg_memory_bank_attention_mask)
        model_inputs['attention_mask'].extend(neg_attention_masks)

        # model_inputs = shuffle_model_inputs(model_inputs)
    ...
    if training_args.do_train:
        ...
        # if os.path.isfile('train_dataset_old.pkl'):
        #     with open('train_dataset_old.pkl', 'rb') as f:
    ...
    if training_args.do_eval:
        print('do_eval的值为', training_args.do_eval)
    ...
    if training_args.do_predict:
        ...
        # if os.path.isfile('predict_dataset.pkl'):
        #     with open('predict_dataset.pkl', 'rb') as f:
    ...
    def postprocess_text_bleu(preds, labels):
        """
        对输入的preds（预测文本）和labels（参考文本）进行处理，使其符合计算BLEU指标的格式要求，包括分词等操作
        """
        tokenized_preds = []
        tokenized_labels = []
        for pred in preds:
            tokenized_pred = nltk.word_tokenize(pred)
            tokenized_preds.append(tokenized_pred)

        for label in labels:
            tokenized_label = [nltk.word_tokenize(label)]
            tokenized_labels.append(tokenized_label)

        return tokenized_preds, tokenized_labels
    def compute_metrics(eval_preds):
        ...
        tokenized_preds, tokenized_labels = postprocess_text_bleu(decoded_preds, decoded_labels)
        print("tokenized_preds", tokenized_preds)
        print("tokenized_labels", tokenized_labels)
        # Some simple post-processing
        decoded_preds, decoded_labels = postprocess_text(decoded_preds, decoded_labels)

        result_bleu = metric_bleu.compute(predictions=tokenized_preds, references=tokenized_labels,smooth=True)
