...
def main():
    ...
    # Ukraine 数据集含预先计算的正负标签列（type），需从 column_names 中移除（由预处理函数单独处理）
    ...
    def preprocess_function(examples):
        ...
        # ── 动态 memory_bank padding ──────────────────────────────
        # 1. 解析所有 memory_bank，得到原始长度
        raw_memory_banks = [ast.literal_eval(mb) for mb in examples['memory_bank']]
        max_kb_len = max((len(mb) for mb in raw_memory_banks), default=1)
        max_kb_len = max(max_kb_len, 1)  # 至少为 1，防止全空时长度为 0

        model_inputs['memory_bank'] = []
        model_inputs['memory_bank_attention_mask'] = []
        model_inputs['has_kb'] = []  # 标记该样本是否有真实 KB（用于对比学习屏蔽）
        for memory_bank in raw_memory_banks:
            has_kb = len(memory_bank) > 0
            model_inputs['has_kb'].append(1 if has_kb else 0)
            # 构建 attention_mask：真实三元组为 1，padding 为 0
            mask = [1] * len(memory_bank) + [0] * (max_kb_len - len(memory_bank))
            # 用 [0,0,0] 将 memory_bank padding 到 max_kb_len
            memory_bank = memory_bank + [[0, 0, 0]] * (max_kb_len - len(memory_bank))
            model_inputs['memory_bank_attention_mask'].append(mask)
    ...
    """
    Ukraine 数据集专用预处理：
    - CSV 已由 neg_sample_ukraine.py 预构建正负样本（type=1/0），直接读取 type 字段
    - 不再在 batch 内重复生成随机负样本，避免污染精心构建的硬/中/易负样本标签
    """
    def preprocess_function_ukraine(examples):
        inputs = examples[text_column]
        targets = examples[summary_column]
        inputs = [prefix + inp for inp in inputs]
        model_inputs = tokenizer(inputs, max_length=data_args.max_source_length, padding=padding, truncation=True)
        ...
        # 直接读取 CSV 中 neg_sample_ukraine.py 预建的 type 字段作为正负标签
        # type=1 → 正样本，type=0 → 负样本（含硬/中/易三级负采样结果）
        model_inputs['neg_pos_label'] = [int(t) for t in examples['type']]
        return model_inputs
