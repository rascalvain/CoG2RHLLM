...
####################################################################################################
def entity_expand_attention_mask(mask: torch.Tensor, input_entity_relation_ids: torch.Tensor, original_attention_mask: torch.Tensor, dtype: torch.dtype):
    """
    Expands attention_mask from `[bsz, seq_len]` to `[bsz, 1, tgt_seq_len, src_seq_len]`.
    """
    bsz, src_len = mask.size()
    tgt_len = original_attention_mask.size()[-1]
...
class MemoryAttention(nn.Module):
    ...
    def forward(
        ...
        hidden_states, attn_weights, _ = self.self_attn(
            ...
            attention_mask=memory_bank_attention_mask,
            layer_head_mask=layer_head_mask,
            output_attentions=output_attentions,
        )
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None, embed_entitys_relations: Optional[nn.Embedding] = None):
        ...
        self.memory_bank_mode = config.memory_bank_mode
        self.use_kg_embedding = config.use_kg_embedding
        self.use_memory_bank = config.use_memory_bank
        
        self.embed_entitys_relations = embed_entitys_relations
        self.embed_entitys_relations.weight.requires_grad = False
        ...
        if original_graph_emb_dim == embed_dim:
            ...
            self.embed_proj = nn.Linear(original_graph_emb_dim, embed_dim, bias=False)
        ...
        if self.use_memory_bank:
            ...
            self.get_memory_attention = MemoryAttention(config)
    ...
    def forward(
        ...
        #################################################################################
        if self.use_kg_embedding:
            entity_relation_embeds = self.embed_entitys_relations(input_entity_relation_ids)
            if self.embed_proj:
                # [bsz, seq_len, orginal_dim] -> [bsz, seq_len, embed_dim]
                entity_relation_embeds = self.embed_proj(entity_relation_embeds)

        if self.use_memory_bank:
            if memory_bank_attention_mask is not None:
                if self.memory_bank_mode == 'all':
                    # [bsz, seq_len=2] -> [bsz, 1, tgt_seq_len, src_seq_len=2]
                    memory_bank_attention_mask = _expand_mask(memory_bank_attention_mask, inputs_embeds.dtype, tgt_len=attention_mask.size()[-1])
                elif self.memory_bank_mode == 'entity':
                    # [bsz, seq_len=2] -> [bsz, 1, tgt_seq_len, src_seq_len=2]
                    
                    memory_bank_attention_mask = entity_expand_attention_mask(memory_bank_attention_mask, input_entity_relation_ids, attention_mask, inputs_embeds.dtype)
                else:
                    print("memory_bank_mode must be all or entity!!")
                    assert False

            memory_bank_embeds = self.embed_entitys_relations(memory_bank)
            if self.embed_proj2:
                memory_bank_embeds = self.embed_proj2(memory_bank_embeds)


        if self.mode == 'input':
            # print('模式为输入之前就处理，此处有逻辑错误')
            # test_mask=head_mask[idx] if head_mask is not None else None
            # print('此处调用的每层的头掩码是什么？',test_mask)
            if self.use_memory_bank:
            ...
            if self.use_kg_embedding and self.use_memory_bank:
                hidden_states = hidden_states + entity_relation_embeds + memory_attention
            elif self.use_kg_embedding:
                hidden_states = hidden_states + entity_relation_embeds
            elif self.use_memory_bank:
                hidden_states = hidden_states + memory_attention
        #################################################################################
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig, entity_relation_weight):
        ...
        self.shared_entity_relation = nn.Embedding(entity_relation_weight.shape[0], original_graph_emb_dim, padding_idx=padding_idx)
        self.shared_entity_relation.weight.data.copy_(torch.from_numpy(entity_relation_weight))
        self.shared_entity_relation.weight.requires_grad = False
        ####################################################################
        
        self.encoder = BartEncoder(config, self.shared, self.shared_entity_relation)
