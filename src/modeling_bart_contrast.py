...
####################################################################################################
def entity_expand_attention_mask(mask: torch.Tensor, input_entity_relation_ids: torch.Tensor, original_attention_mask: torch.Tensor, dtype: torch.dtype):#扩展到和原始的注意力掩码的长度一致
    """
    Expands attention_mask from `[bsz, seq_len]` to `[bsz, 1, tgt_seq_len, src_seq_len]`.
    """
...
class BartEncoder(BartPretrainedModel):#改写Bart模型的编码器部分，具体来说就是对输入n层transformer的起始序列做文章
    ...
    def forward(
        ...
        subgraph_embeds=self.r_proj_subgraph(subgraph_embeds.permute(0,1,3,2)).squeeze() #[batch_size,triple num,768]
        #原本的操作是将每个memory bank展平再做映射
        # subgraph_embeds=subgraph_embeds.reshape(subgraph_embeds.size(0),-1)
        # self.r_proj_subgraph1= nn.Linear(subgraph_embeds.size(1),self.embedding_dim,bias=False,device=device)
        # subgraph_embeds=self.r_proj_subgraph1(subgraph_embeds)
        #这里我们参考ICML的操作对三元组向量直接求平均值得到平均表示
